...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(!hasPermission || typeof hasPermission === 'string') {
			...
			if(typeof hasPermission === 'string') return this.channel.send(hasPermission);
			else return this.channel.send(`You do not have permission to use the \`${this.command.name}\` command!`);
		}
		...
		if(this.message.channel.type === 'text' && this.command.clientPermissions) {
			...
			if(missing.length > 0) {
				...
				return this.channel.send(oneLine`
		...
		if(!args && this.command.argsCollector) {
			...
			if(result.cancelled) {
				if(result.prompts.length === 0) {
					...
					return this.channel.send(err.message);
				}
				return this.channel.send('Cancelled command!');
			}
			args = result.values;
		}
