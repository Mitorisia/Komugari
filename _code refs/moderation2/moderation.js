            ...
            if (!ctx.msg.member.permission.has('kickMembers')) {
                ...
                if (!ctx.guildBot.permission.has('kickMembers')) {
                    ...
                    if (ctx.mentions.length === 0) {
                        ...
                        var promises = [];
                        for (let u of ctx.mentions) {
                            if (u.id !== bot.user.id) {
                                if (u.id !== ctx.msg.author.id) {
                                    promises.push(bot.kickGuildMember(ctx.guild.id, u.id).catch(err => {
            ...
            if (!ctx.msg.member.permission.has('banMembers')) {
                ...
                if (!ctx.guildBot.permission.has('banMembers')) {
                    ...
                    if (ctx.mentions.length === 0) {
                        ...
                        var promises = [];
                        ctx.mentions.forEach(u => {
                            if (u.id !== bot.user.id) {
                                if (u.id !== ctx.msg.author.id) {
                                    promises.push(bot.banGuildMember(ctx.guild.id, u.id, 7).catch(err => {
                                        if (err.resp.statusCode === 403) {
